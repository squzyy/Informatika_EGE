...
'''
Все пятибуквенные слова, в составе которых могут быть только буквы Л, Е, Й, К, А, записаны в некотором порядке и пронумерованы начиная с 1.
...
Под каким номером в списке идет последнее слово, которое содержит не более одной буквы К и не содержит букв Е стоящих рядом.
from itertools import *
c = 0
for i in product('АЙКЛЕ', repeat = 5):
    s = ''.join(i)
    c = c + 1
