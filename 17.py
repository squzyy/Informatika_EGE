...
"""
содержится последовательность целых чисел. Элементы последовательности могут принимать целые значения от –100 000 до 100 000 включительно. Определите количество пар последовательности, в которых только одно число отрицательно, а сумма чисел пары не больше количества чисел в последовательности, оканчивающихся на 5. В ответе запишите количество найденных пар, затем через пробел максимальную из сумм элементов таких пар. В данной задаче под парой подразумевается два идущих подряд элемента последовательности.
a = [int(i) for i in open('17.txt')]

l = len([i for i in a if abs(i) % 10 == 5])
otv = []
for i in range(len(a) - 1):
    x, y = a[i], a[i+1]
